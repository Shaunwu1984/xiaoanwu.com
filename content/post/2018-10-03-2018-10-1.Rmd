...
昨天晚上（9月30日）还岁月静好，但是10月1号早上起来就发现电脑系统奔溃了，这是个悲伤的故事，意味着很多东西得从头再来。但是也是好的锻炼，电脑的各项技能有得到了新的提高，比如如何翻墙，如何下载安装软件，都比以前更有序了，也理解地更好了。所以是好事也是坏事。
但是就是两天就这样白白耽误了，我得赶紧看书，因为明天就要讨论Maudlin的那篇文章“Causation, Counterfactual, and the Third Factor”。
...
1. the sort of physical world I have described at least could be a world that has laws of the evolution of the distribution of matter.  （我并不明白他这个澄清想表达什么！）

2. 关于物质分布演化的所有反事实真值都是确定的。

对于上述**问题**有两条回应的路径：

1. 直觉诉诸于Hume counterfactual，拒绝在那个位置的物质是原因。

    + 直接因果。
    + 直接因果的传递闭包。
但是在我们所讨论的情形中只有直接因果。所以如果没有反事实依赖就没有因果。

2. 认为底层中心物质是原因。

首先指出，一般而言，这个世界的演化规则似乎把完备的$3 \times 3$占有形式和中心方格的后继状态联系在一起。如果中心底层位置的状态从来没有和演化相关，那么对于它的因果效应就应该没有任何可以争议的地方。

上述例子是为了说明，我们对于相关的反事实没有任何异议，但是对于因果的判断却有合理的争议。

## 四. 自然律的作用

至少要给出这些反事实论断和因果论断的真值条件是如何进行的。
...
我们称定律是拟-牛顿定律（**quasi-Newtonian laws**），如果一方面，惯性定律描述了当没有任何东西作用于它们，这些实体将如何行为，另一方面，有偏离定律，确切说明了，在什么条件下，以何种方式，行为将会偏离它的惯性行为。


定律不需要是拟-牛顿定律。在基础的层面上，生命游戏定律就不是拟-牛顿定律。但这当然并不意味着生命游戏没有原因。存在原因和结果，但是在基础的层面上，我们设想它们的方式和我们在拟-牛顿定律的背景中设想原因的方式是不同的。

“our natural desire is to think of the world in quasi‐Newtonian terms, in terms of inertial behavior (or ‘natural’ behavior) and deviations from inertial behavior: in terms of, to use a concept from mathematical physics, perturbation theory.” （p.15）
...
但是我们并不是通过思考物理定律来理解我们确实理解的大量东西。比如我理解很多我现在使用的电脑是如何工作的，但是很少这些理解是得自机器物理的细节知识。而是这样的，我是通过某种类律的推广来思考它而理解它的运转的。
...
点就在于，这就是特定科学如何运作的：“they seek to impose a taxonomy on the physical structure of the world  in such a way that the objects as categorized by the taxonomy  fairly reliably obey some lawlike generalizations which can be stated in terms of the taxonomy.
它们试图给世界的物理结构加诸一种分类学，以这种方式，被分类学所分类的对象相当可信赖地遵守着某些可以通过分类学而得到表达的类律的推广”，关于电脑如何工作的推广显然就是这一类。

根据某些节点划分自然（**carving nature at the joints**）：“finding a macrotaxonomy such that there are reasonably reliable and informative and extensive lawlike generalizations which can be stated in terms of the taxonomy”。

>这里有几个概念把我搞晕了：taxonomy, speical sciences, quasi-Newtonian, lawlike generalizations。似乎是这样一个顺序：different taxonomies $\to$ different lawlike generalizations $\to$ different causal judgement.



例子：通过拟-牛顿概念来理解电脑，通过拟-牛顿概念来理解 human biology，我们以拟-牛顿的形式来考虑神经元的运行。

特定科学不是根本的一个很大优势在于：这为构建被拟-牛顿扩展所很好描述的宏观分类学提供了自由度。同时它也提供了划分节点的自由度：我们可以转移边界以便确定遵循更为稳健推广的系统。

我伸手接一个球，沿着球运动的方向后面是一堵水泥墙，墙的后面是窗户。
大部分人不认为我伸手接住球是球未能撞击的原因，理由就是反事实条件句为假：“如果我没有接球，球会撞击到玻璃”为假。但是如果我们把墙换成一个100% 可靠的接球手，尽管这一次反事实依然为假，但是大部分人会认为认为我伸手接住球是球未能撞击的原因。

为什么会有这种差别？

我们**carve the situation up into systems**的不同所导致的。

1. 如果我们考虑寻常的玻璃自身，它的惯性行为就是保持不破。被称为“威胁”的那类事物就是有充分的质量，硬度和速度的对象。

2. 称玻璃+水泥墙的系统为“被保护的窗户（protexted window）”。在被保护的窗户系统中，窗户的惯性状态是保持不破，“威胁”就是那些能够穿透墙的东西。
